...
ind = ind.drop(axis = 1, labels = 'male')
...
# Group and aggregate
ind_agg = ind.drop(axis = 1, labels = 'Target').groupby('idhogar').agg(['min', 'max', 'sum', 'count', 'std', range_])
ind_agg.head()
...
final = heads.join(ind_agg, on='idhogar', how = 'left')
...
final = final.merge(head_gender, on = 'idhogar', how = 'left')
final=final.rename(columns ={'female': 'female_head'})
...
cv_score = cross_val_score(model, train_set.astype(str), train_labels.astype(str), cv = 10, scoring = scorer)
